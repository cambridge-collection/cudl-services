...
describe(`metadata routes /:format/:id`, () => {
  test('/json/:id reponds with JSON metadata', async () => {
    const response = await request(getTestApp()).get(`/json/MS-ADD-03959`);
  ...
  test('/json/:id responds with 404 for missing ID', async () => {
    const response = await request(getTestApp()).get(`/json/MS-MISSING`);
    expect(response.status).toBe(NOT_FOUND);
  });

  test('/json/:id responds with 500 for invalid metadata', async () => {
    const response = await request(getTestApp()).get(`/json/INVALID`);
  ...
  test.each([
    ...
    const response = await request(getTestApp()).get(url);
    expect(response.status).toBe(BAD_REQUEST);
  });

  test('/:format/:id responds with 500 for invalid metadata when requesting non-json metadata', async () => {
    const response = await request(getTestApp()).get(`/tei/INVALID`);
    expect(response.status).toBe(INTERNAL_SERVER_ERROR);
  });
