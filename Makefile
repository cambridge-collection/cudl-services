...
github-actions-set-docker-image-tag-outputs:
	@echo "::set-output name=IMAGE_TAG_COMMIT_SHA::camdl/cudl-services:$(COMMIT_SHORT_HASH)"
	@if [ "$(COMMIT_TAG)" != "" ]; then \
		echo "::set-output name=IMAGE_TAG_COMMIT_TAG::camdl/cudl-services:$(COMMIT_TAG)" ; \
	fi
