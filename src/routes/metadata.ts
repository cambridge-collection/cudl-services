...
import {CORS_HEADERS, isExternalCorsRequest, isSimplePathSegment} from '../util';

export type MetadataOptions = Pick<Config, 'dataDir'>;

export function getRoutes(options: {router?: express.Router} & MetadataOptions) {
...
function createMetadataHandler(dataDir: string) {
    return expressAsyncHandler(async (req: express.Request, res: express.Response, next) => {
        ...
        if (req.params.format === 'json') {
            ...
            if(isExternalCorsRequest(req) && data.embeddable === false) {
            ...
            if (data?.descriptiveMetadata?.[0]?.metadataRights?.trim())  {
                ...
                res.sendFile(path.join(dataDir, 'data', req.params.format, req.params.id, `${req.params.id}.xml`));
            } else {
                res.status(FORBIDDEN).json({
                    error: util.format(
...
async function loadJsonMetadata(path: string): Promise<any> {
    try {
