...
>
  ...
  <xsl:function name="cudl:elem-empty-in-normalised-view" as="xs:boolean">
    ...
    <xsl:value-of select="xs:boolean(($viewMode = 'normalised') and ($node = $node//tei:del[not(contains(@type,'redacted'))]))"/>
  </xsl:function>
  ...
  <xsl:function name="cudl:contains-text-or-displayable-elem" as="xs:boolean">
    ...
    <xsl:value-of select="xs:boolean(($node/normalize-space() or $node/*[not(local-name()=($special_app_children, $non_displayable_elems))]))"/>
  </xsl:function>
