...
<!--            <xsl:attribute name="style" select="'text-align: center;'"/>-->
   ...
   <xsl:template match="*:lb" mode="html">
      ...
      <xsl:if test="//*:lb[@n]">
         ...
         <xsl:variable name="lineNumberLength">
            <xsl:value-of select="string-length(@n)"></xsl:value-of>
         </xsl:variable>

         <!--<xsl:value-of select="$lineNumberLength"></xsl:value-of>-->

         <xsl:variable name="lineNumber">

            <xsl:choose>
               <xsl:when test="$lineNumberLength=0">
               ...
               <xsl:when test="$lineNumberLength=1">
         ...
         <xsl:element name="span">
            ...
            <xsl:attribute name="style" select="'font-size:smaller;'" />
      ...
      <xsl:choose>
         <xsl:when test="normalize-space(@from)">
            ...
            <xsl:variable name="page">
               <xsl:for-each select="//*:text/*:body/*:div/*:pb" >
                  <xsl:if test="@n = $from">
            ...
            <xsl:element name="a">
               <xsl:attribute name="href" select="concat('#PHYS-', $page)"/>
               <xsl:apply-templates mode="html" />
            </xsl:element>
         </xsl:when>
         <xsl:otherwise>
...
-->
   ...
   <xsl:template match="*:hi[@rend='small']" mode="html">
   ...
   <xsl:template match="*:hi[@rend='underline']" mode="html">
   ...
   <xsl:template match="*:hi[@rend='underlineDashed']" mode="html">

      <xsl:element name="span">

         <xsl:attribute name="style" select="'border-bottom: 1px dotted #000;'" />
         <xsl:apply-templates mode="html" />
      </xsl:element>
   ...
   <xsl:template match="text()" mode="html">
      ...
      <!--these are hacks to cope with some of the sanskrit transcription - find alternative ways-->
      <xsl:variable name="translated" select="translate(., '&#x00A7;', '&#x30FB;')" />

      <!--replace more than one caret with thre nbsps-->
      <xsl:variable name="translated2" select="replace($translated, '\^{2,}', '&#160;&#160;&#160;')"/>

      <!--<xsl:variable name="translated" select="translate(., '^&#x00A7;', '&#x00A0;&#x30FB;')" />-->
<!--      <xsl:variable name="replaced" select="replace($translated, '&#x005F;&#x005F;&#x005F;', '&#x2014;&#x2014;&#x2014;')" /> -->
      <xsl:variable name="replaced" select="replace($translated2, '_ _ _', '&#x2014;&#x2014;&#x2014;')" />
